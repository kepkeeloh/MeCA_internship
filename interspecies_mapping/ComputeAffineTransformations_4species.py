...
def Affine_composition(modelP1, modelP2, modelP3, model P4, corrTableP12, corrTableP23, corrTableP34, corrTableP13, corrTableP14):
    """
    ...
    ## P3 to P4
    print('computing affine transformations from ' + Primate3 + ' to ' + Primate4)

    Ncorr_lonP34 = len(corrTableP34['lon_' + Primate3])
    Ncorr_latP34 = len(corrTableP34['lat_' + Primate3])
    long_corrP34 = np.zeros((Ncorr_lonP34, 2)).astype('int')
    lat_corrP34 = np.zeros((Ncorr_latP34, 2)).astype('int')
    for i in range(Ncorr_lonP34):
        long_corrP34[i][0] = longID_P3[sulci_lon_P3[corrTableP34['lon_' + Primate3][i]]]
        long_corrP34[i][1] = longID_P4[sulci_lon_P4[corrTableP34['lon_' + Primate4][i]]]
    for i in range(Ncorr_latP34):
        lat_corrP34[i][0] = latID_P3[sulci_lat_P3[corrTableP34['lat_' + Primate3][i]]]
        lat_corrP34[i][1] = latID_P4[sulci_lat_P4[corrTableP34['lat_' + Primate4][i]]]

    # make the lists of the axes that have a correspondence (and therefore define the intervals)
    longP34 = np.sort(np.concatenate(([0], sulciP3[0][long_corrP34[:, 0]], [360])))
    latP34 = np.sort(np.concatenate(([insP3], sulciP3[1][lat_corrP34[:, 0]], [cinP3])))
    longP43 = np.sort(np.concatenate(([0], sulciP4[0][long_corrP34[:, 1]], [360])))
    latP43 = np.sort(np.concatenate(([insP4], sulciP4[1][lat_corrP34[:, 1]], [cinP4])))

    P3toP4 = Affine_transform([longP34, latP34], [longP43, latP43])
